...
RegisterNetEvent('RemoveCivChannels')
AddEventHandler('RemoveCivChannels',function()
  ...
  exports.tokovoip_script:removePlayerFromRadio(25)
end)

RegisterNetEvent('RemovePoliceChannels')
AddEventHandler('RemovePoliceChannels',function()
...
function OpenRadioMenu()
  ...
  local elements = {
    ...
    if data.current.value == 'leavepublic' then
      TriggerEvent("RemoveCivChannels")
    end 
    if data.current.value == 'leavepolice' then
      TriggerEvent("RemovePoliceChannels")
    end 	  
